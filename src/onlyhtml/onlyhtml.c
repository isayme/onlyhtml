...
static int32_t setnonblocking(int sockfd);
static int32_t create_and_bind (uint16_t port);

static int32_t file_len(int fd);
static char *get_http_url(char *req);
static int32_t handle_request(int sockfd);
static int32_t respond_404(int sockfd);
...
static int32_t respond_404(sockfd)
{
    static int fd = -1;
